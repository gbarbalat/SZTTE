...
make_table1 <- function(merged_ignore, merged_ignore_full) {
  ...
  if (!is.null(merged_ignore$BEN_IDT_ANO)) merged_ignore$BEN_IDT_ANO <- NULL
  if (!is.null(merged_ignore$DCD_CIM_COD)) merged_ignore$DCD_CIM_COD <- NULL
...
# ---- Run it ----
table1 <- make_table1(merged_ignore, merged_listwise)
print(table1, n=Inf)
